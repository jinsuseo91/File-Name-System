...
class FileNameTool(tb.Window):
    ...
    def create_widgets(self):
        ...
        tb.Button(header_frame, text="초기화", bootstyle="info", style="Bold.TButton",
                  command=self.reset_fields).pack(side=RIGHT)

        # 카드 배경색 (Entry/Combobox와 일치)
        card_bg = "#ffffff"  # 살짝 밝은 회색, Entry 배경과 동일
        border_color = "silver"
        border_width = 1
        ...
        file_card = tk.Frame(container, bg=card_bg, highlightbackground=border_color,
                             highlightthickness=border_width, padx=12, pady=12)
        file_card.pack(fill=X, pady=6)
        tb.Label(file_card, text="파일 선택 (선택사항)", font=("Segoe UI", 13, "bold"), background=card_bg).pack(anchor=W)
        self.file_label = tb.Label(file_card, text="선택된 파일 없음", foreground="#6B7280", background=card_bg)
        self.file_label.pack(side=LEFT, pady=6)
        tb.Button(file_card, text="파일 선택", bootstyle="info", style="Bold.TButton",
        ...
        def row(label, widget, r):
            tb.Label(input_card, text=label, font=("Segoe UI", 10, "bold"), background=card_bg).grid(
                row=r, column=0, sticky=W, pady=6, padx=6)
            widget.grid(row=r, column=1, sticky=W)

        entry_style = {"bootstyle": "secondary"}  # Entry/Combobox 배경 통일

        # 대분류
        self.main_category = tb.Combobox(input_card, values=["HW","SW","SYS","ETC","기타"], width=28, **entry_style)
        self.main_category.set("HW")
        row("대분류", self.main_category, 0)
        self.main_category_custom = tb.Entry(input_card, width=30, state="disabled", **entry_style)
        self.main_category_custom.grid(row=0, column=2, padx=6)

        # 중분류
        self.sub_category = tb.Combobox(input_card, values=["IPC","NVR","DVR","A-CAM","VMS","Parking","기타"], width=28, **entry_style)
        self.sub_category.set("IPC")
        row("중분류", self.sub_category, 1)
        self.sub_category_custom = tb.Entry(input_card, width=30, state="disabled", **entry_style)
        ...
        # 문서유형
        self.doc_type = tb.Combobox(input_card, values=["사용자매뉴얼","사양서","가이드","릴리즈노트","장애조치","기타"], width=28, **entry_style)
        self.doc_type.set("사용자매뉴얼")
        row("문서유형", self.doc_type, 3)

        self.doc_type_custom = tb.Entry(input_card, width=30, state="disabled", **entry_style)
        ...
        # 결과 카드
        # -----------------------------
        result_card = tk.Frame(container, bg=card_bg, highlightbackground=border_color,
                               highlightthickness=border_width, padx=12, pady=12)
        result_card.pack(fill=BOTH, expand=True, pady=6)
        tb.Label(result_card, text="생성된 파일명", font=("Segoe UI", 10, "bold"), background=card_bg).pack(anchor=W)
        ...
        # NAS / 문서관리대장 링크 카드
        # -----------------------------
        link_card = tk.Frame(container, bg=card_bg, highlightbackground=border_color,
                             highlightthickness=border_width, padx=12, pady=12)
        link_card.pack(fill=X, expand=False, pady=(6,0))
        tb.Label(link_card, text="빠른 접근", font=("Segoe UI", 10, "bold"), background=card_bg).pack(anchor=W, pady=(0,6))
    ...
    def open_nas(self):
        nas_path = "http://csn.idis.co.kr:5500/"
        try:
            webbrowser.open(nas_path)
        except Exception as e:
            messagebox.showerror("오류", f"NAS 경로를 열 수 없습니다: {e}")

    def open_sheet(self):
        sheet_url = "https://docs.google.com/spreadsheets/d/1rzqQZc9pfW_gxlifbtZJRPO4YsHl1fGpAaeuL4I-PC8/edit?gid=0#gid=0"
